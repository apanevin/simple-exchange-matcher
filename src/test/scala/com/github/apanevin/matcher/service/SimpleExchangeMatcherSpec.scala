...
class SimpleExchangeMatcherSpec extends FlatSpec with Matchers {
  ...
  it should "process simple exchange" in {
    ...
    val updated = new SimpleExchangeMatcher().process(clients, orders.toIterator)
    updated(0) should equal(Client("C1", 100, 0, 0, 0, 0))
    updated(1) should equal(Client("C2", 0, 5, 0, 0, 0))
  }

  it should "process complex exchange (selling first)" in {
    val clients = List(
      Client("C1", 0, 5, 0, 0, 0),
      Client("C2", 100, 0, 0, 0, 0),
  ...
  it should "process complex exchange (buying first)" in {
    val clients = List(
      Client("C1", 0, 5, 0, 0, 0),
      Client("C2", 100, 0, 0, 0, 0),
      Client("C3", 60, 0, 0, 0, 3)
